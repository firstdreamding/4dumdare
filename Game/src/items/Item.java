...
public class Item {
	...
	static public enum List {
		KNIFE("Knife", 1, 1, 0, 1, 3, 200, WEAPON, "/sprites/ItemKnife.png"),
		FEDORA("Fedora", 0, 1, 2, 2, 2, 200, COSMETIC, "/sprites/ItemFedora.png"),
		AWP("AWP", 2, 0, 0, 0, 3, 700, WEAPON, "/sprites/ItemAWP.png");
		public int att, def, loy, luc, acc, price, type;
		public String path;
		public Texture t;

		List(String name, int att, int def, int loy, int luc, int acc, int price, int type, String path) {
			this.att = att;
			this.def = def;
			this.loy = loy;
			this.luc = luc;
	...
	public Item(String s) {
		System.out.println(List.valueOf(s.toUpperCase()));
		List l;
		l = List.valueOf(s.toUpperCase());
		att = l.att;
		def = l.def;
		loy = l.loy;
		luc = l.luc;
